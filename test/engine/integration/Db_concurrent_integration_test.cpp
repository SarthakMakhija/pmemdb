...
TEST_F(DbFixture, DbConcurrentIntegration_TwoThreadsReadingTheSameKey) {
    ...
    std::thread reader1([&]() {
        ASSERT_EQ(Slice("Hard disk drive"), DbFixture::getDb() -> get(Slice("HDD")).first);
    });
...
TEST_F(DbFixture, DbConcurrentIntegration_TwoThreadsReadingDifferentKeys) {
    ...
    std::thread reader2([&]() {
        ASSERT_EQ(Slice("Persistent Memory"), DbFixture::getDb() -> get(Slice("Pmem")).first);
    });
...
TEST_F(DbFixture, DbConcurrentIntegration_TwoThreadsPerformingUpdateOnSameKey) {
    ...
    Slice value = DbFixture::getDb() -> get(Slice("HDD")).first;
    ASSERT_TRUE(value == Slice("Hard disk") || value == Slice("HDD"));
}
...
TEST_F(DbFixture, DbConcurrentIntegration_TwoThreadsPerformingPutAndDeleteOnSameKey) {
    ...
    Slice value = DbFixture::getDb() -> get(Slice("HDD")).first;
    ASSERT_TRUE(value == Slice("Hard disk drive") || value == Slice(""));
}