...
TEST(SkipListException, SkipListException_ThrowInvalidArgumentExceptionGivenTowerSizeIsZero) {
    ASSERT_THROW(new SkipList(new LevelGenerator(0), new StringKeyComparator(), nullptr), std::invalid_argument);
}

TEST(SkipListException, SkipListException_ThrowInvalidArgumentExceptionGivenTowerSizeIsLessThanZero) {
    ASSERT_THROW(new SkipList(new LevelGenerator(-1), new StringKeyComparator(), nullptr), std::invalid_argument);
}